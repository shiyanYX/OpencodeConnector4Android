...
- **应用内更新检测** — 启动时自动检查 GitHub 是否有新版本，一键下载安装 APK（国内镜像代理）
- **后台 SSE 保活** — 前台 Service 在 App 后台时保持连接不断开
- **App 被杀后状态不丢失** — 问题/权限确认气泡、流式输出进度通过磁盘缓存持久化；App 被系统杀死后重新打开，会话状态完整恢复
- **统一聊天设置** — 通过输入栏的 Settings 对话框按会话配置 Agent、Model 和 Variant
- **Variant 选择** — 当服务端提供变体时，可选择特定模型变体（如量化级别）
- **基于时间的会话分组** — 会话按 今天/昨天/本周/更早 分组显示，配相对时间戳
- **子会话树** — 在父会话下展开/折叠子会话的树形布局
- **会话搜索** — 防抖搜索栏，按标题过滤会话
- **活跃会话状态点** — 实时绿色脉冲圆点指示忙碌会话
- 实时 AI 聊天，SSE 流式输出
- 工具调用渲染（思考气泡、工具摘要）
- Todo 任务面板浮层
...
1. 打开 OConnector → 在服务器列表页点击 **+** 添加第一台服务器
2. 输入 PC 的 IP、端口和凭据 → 点击 **连接**
3. 会话列表自动展示 PC 上所有项目的会话
4. 点击项目查看其会话，点击会话进入聊天
5. 点击顶部眼睛按钮，可切换隐藏/显示子会话
6. 发送消息、切换 Agent、查看 Todo
